<template>
  <div class="columns is-multiline">
    <div class="column is-12 is-4-desktop">
      <div class="mb-6 is-flex" v-for="(comment,name) in this.comments" :key="name">
              <span>
        ...
        <div class="ml-3">
          <h4 class="is-size-4 has-text-weight-bold mb-2">@{{name}}</h4>
          <p class="subtitle has-text-grey">{{comment}}</p>
        </div>
      </div>
    </div>
  </div>
